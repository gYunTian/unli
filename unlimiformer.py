...
class Unlimiformer(Generic[ModelType]):
    ...
    def pre_forward_hook(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        ...
        if not self.is_input_encoding_pass:
            if self.model.training:
                ...
                print("After chunked encode: ", self.long_inputs_encoded.shape, self.long_inputs_mask.shape)
                ...
                print("After slicing: ", input_ids.shape, attention_mask.shape, "\n")
    ...
    def create_cross_attn_pre_forward_hook(self, original_cross_attn_forward_func, decoder_layer, i):
        def attention_pre_forward_hook(hidden_states, attention_mask=None, *args, **kwargs):
            ...
            if self.model.training:
                ...
                print("Attention pre forward hook")

                # from: (batch, tgt_len, dim) to: (batch * tgt_len, 1, dim)
                batch_size, tgt_len, dim = hidden_states.shape
                print("Before states reshaping: ", hidden_states.shape, attention_mask.shape)
                ...
                print("After states reshaped: ", hidden_states.shape, attention_mask.shape)
                ...
                print("After cross attn (attn_output, attn_weight, past_key_value): ", \
                  attn_output.shape, attn_weights_reshaped.shape, past_key_value.shape)
                
                attn_output = attn_output.reshape(batch_size, tgt_len, dim)
                print("attn output reshaped: ", attn_output.shape, "\n")
                
                return attn_output, attn_weights_reshaped, past_key_value
            else:
                print("Attention pre forward hook")
                print("Original cross attn forward \n")
    ...
    def attention_forward_hook(self, module, input, output):
        ...
        print("In attention forward hook")

        with torch.no_grad():
            
            print("Before process query: ", query.shape)

            query = self.process_query(output)[:,-1] # (batch * beam, head, dim)
            print("Query processed: ", query.shape)
            query = query[:, self.head_nums] # (batch * beam, head, dim)
            print("QUery sliced: ", query.shape)
            ...
            if self.use_datastore:
                ...
                if self.reconstruct_embeddings:
                    print("In search and reconstruct embeddings")
                    ...
                    print("top_search_key_scores: ", top_search_key_scores.shape)
                    print("top_search_key_indices: ", top_search_key_indices)
                ...
                print("Embds reshaped: ", embeddings.shape)
        ...
        if self.use_datastore:
            ...
            print(k_proj_layer.weight.shape, k_proj_layer.bias.shape, v_proj_layer.weight.shape, v_proj_layer.bias.shape)

            embed_dim = embeddings.shape[-1]
            print("Embed dim: ", embed_dim.shape)
        ...
        print("Cur layer key value placeholder: ", self.cur_layer_key_value_placeholder)
        print("new_keys flatten: ", new_keys.flatten(0,1))
        print("new values flatten: ", new_values.flatten(0, 1))
